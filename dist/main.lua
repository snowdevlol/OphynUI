	...
	local function runKeyCheck(key)
		...
		if not (result and result.ok and result.valid) then
			...
			if result and result.ok then
				notify("Invalid key", "That key isn't valid. Get a new one.", "error")
			else
				notify("Something went wrong", "Couldn't check your key. Try again.", "error")
			end
			task.wait(0.35)
			state.ready = true
			checkingKey = false
