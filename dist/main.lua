...
function UI.new(options)
	...
	local INVITE_DISPLAY, INVITE_URL, INVITE_CODE = "", "", ""
	if HAS_DISCORD then
	...
	local WEBSITE_DISPLAY, WEBSITE_URL = "", ""
	if HAS_WEBSITE then
	...
	local website = linkButton(HAS_DISCORD and 206 or 158, "Website", WEBSITE_DISPLAY, Images.LINK)
	discord.Visible = HAS_DISCORD
	website.Visible = HAS_WEBSITE
	...
	local LAYOUT = {
		collapsed = { divider = 148, discordY = 158, discordH = 40, websiteY = HAS_DISCORD and 206 or 158 },
	...
	local function runKeyCheck(key)
		...
		task.spawn(function()
			...
			if type(cfg.Callback) == "function" then
				local ok, err = pcall(cfg.Callback, key)
				if not ok then
					runError = err
				end
			end
...
-- Thin wrapper around the Jnkie SDK (https://docs.jnkie.com/roblox-sdk).
-- Only handles talking to the SDK: loading it, configuring service/identifier/
-- provider, checking keys and fetching "get key" links. It knows nothing about
-- any particular UI; components/window/ui.lua plugs this in as a
-- KeySystem.Key validator function.
...
-- config = { Service = "...", Identifier = "...", Provider = "Mixed" }
function Jnkie.new(config)
	config = config or {}
	local self = setmetatable({}, Jnkie)
	self.__isJnkieInstance = true -- lets ui.lua auto-detect a raw instance passed as KeySystem.Key
...
-- The docs are inconsistent about which field carries the pass/fail flag
-- ("valid" in the documented response shapes, "success" in their own example
-- code, plus a couple of known success messages) — so check all of them
-- instead of trusting a single field name.
...
-- Returns a table shaped like { valid = bool, message = string?, error = string? }
...
-- Returns link, err — mirrors Junkie.get_key_link()
...
-- Adapter for KeySystem.Key: callable like a function(key) -> valid, reason,
-- so a Window can be configured with:
--   KeySystem = { Key = Jnkie.new({ Service = "...", Identifier = "...", Provider = "Mixed" }):Validator() }
-- It's a callable *table* rather than a plain function so the Window can spot
-- the __isJnkie marker and reach back into __instance to auto-wire "Get a key"
-- to Jnkie:GetKeyLink() without the user having to set KeySystem.URL by hand.
